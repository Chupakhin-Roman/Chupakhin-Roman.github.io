	...
	add_validator: function(id, name, group_names, required){
		$(function(){
			var el = $(":input[name=" + name + "]", CKM5.form_selector);
			if (el.size() == 0 && group_names) {
				for (var n in group_names)
					$(":input[name=" + group_names[n] + "]", CKM5.form_selector).attr("rel", name);
				el = $(":input[rel=" + name + "]", CKM5.form_selector);
			}
			el.data("field_id", id);
			if (required) {
				el.addClass(CKM5.required_class);
				el.data("error", required);
			} else
				el.addClass(CKM5.ajax_class);
		});
	},
	prepop_field: function(field, value, group_names){
		$(function(){
			var el = $(":input[name=" + field + "]", CKM5.form_selector);
			if (el.size() == 0 && group_names) {
				for (var n in group_names) {
					el = $(":input[name=" + group_names[n] + "]", CKM5.form_selector);
					if (el.val().length > 0 && !el.hasClass(CKM5.default_value_class))
						break;
			...
			} else if (el.val().length == 0 || el.hasClass(CKM5.default_value_class))
				el.val(value);
		});
	},
	format_valid: function(input){
		input.removeClass(CKM5.error_class).addClass(CKM5.valid_class);
	},
	format_invalid: function(input, err){
		input.removeClass(CKM5.valid_class).addClass(CKM5.error_class);
	},
	remove_format: function(input){
		input.removeClass(CKM5.error_class).removeClass(CKM5.valid_class);
	},
...
$().ajaxError(function(e, req, s){
	if (s.url.indexOf("v.ashx") > -1) {
		var field = $("#" + s.url.substring(s.url.indexOf("v.ashx") + 9).split("&")[0], CKM5.form_selector);
		field.removeData("changing").removeData("invalid").removeData("valid");
		CKM5.remove_format(field);
	} else {
		CKM5.submitting = false;
		CKM5.popup_error(CKM5.ajax_error);
		$(CKM5.submit_selector, CKM5.form_selector).attr("disabled", false);
	}
});
$(function(){
	$.ajaxSetup({timeout: CKM5.ajax_timeout});

	var ckm_form = $(CKM5.form_selector);
	...
	$(":text,:password,textarea", ckm_form).change(function(){
		...
		if ($.trim(me.val()).length == 0 || me.hasClass(CKM5.default_value_class)) {
			...
			if (group) {
				...
				if (partial || me.hasClass(CKM5.required_class))
					format_invalid(me, me.data("error") || CKM5.required_error);
				else
					format_valid(me);
			}
			else if (me.hasClass(CKM5.required_class))
				format_invalid(me, me.data("error") || CKM5.required_error);
			else
				format_valid(me);
		} else if (me.hasClass(CKM5.ajax_class)) {
			var val = me.val();
			var group = me.attr("rel");
			if (group) {
				val = "";
	...
	$("select", ckm_form).change(function(){
		var me = $(this);
		if (me.hasClass(CKM5.required_class) && (me.val() == null || me.val() == "" || me.hasClass(CKM5.default_value_class)))
			format_invalid(me, me.data("error") || CKM5.required_error);
		else
			format_valid(me);
	});
	...
	if (CKM5.use_iframe) {
		...
		CKM5.on_iframe_load = function(data){
		...
		if (data) {
			if (data.success) {
				if (data.nextEval)
					...
					CKM5.change_page(data.nextPage);
			} else {
				if (data.error)
					CKM5.popup_error(CKM5.error_title, data.error);
				else {
					...
					CKM5.popup_error(CKM5.error_title, CKM5.validation_error);
				}
			}
		} else
			CKM5.popup_error(CKM5.ajax_error);
			
		$(CKM5.submit_selector, ckm_form).attr("disabled", false);
		CKM5.submitting = false;
		...
		if (!CKM5.submitting) {
			...
			$(":text,select,:password,textarea", ckm_form).each(function(){
				...
				if (me.data("changing")) {
					...
					if (me.hasClass(CKM5.required_class) || me.val().length > 0) //allow optional fields retry
						valid = false;
			...
			$(CKM5.submit_selector, ckm_form).attr("disabled", true);
